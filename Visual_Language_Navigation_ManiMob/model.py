...
from KB.ConceptNet_MMKG import GCN_pre_define
...
class TransformerEncoder(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        if self.use_glove:
            ...
            self.embedding.weight.data[...] = torch.from_numpy(glove)
            self.embedding.weight.requires_grad = False
        encoder_layer = torch.nn.TransformerEncoderLayer(d_model=embedding_size, nhead=nhead, dropout=dropout_ratio,
                                                         dim_feedforward=ff)
...
class object_roomTransformer(nn.Module):
    # fake code:
    # 1.language_input->attn_with h_0 get text_object_attn(1*512),text_room_attn(1*512)
    # 2.text_object_attn(1*512) -> attn_weight with knowledge base get top K(5*512)
    ...
    def __init__(self, embedding_size, hidden_size, dropout_ratio,
                 ...
                 wo_instr_input=False, transformer_dropout_rate=0.1, action_prediction_mode='single', label_size=512,
        ...
        num_rooms = 31
        repeat = 128
        decoder_layer = nn.TransformerDecoderLayer(hidden_size, num_head, dropout=transformer_dropout_rate,
                                                   dim_feedforward=ff)
        ...
        self.object_map = nn.Linear(hidden_size, 512)
        ...
        for p in self.text_room_classifier.parameters():
            p.requires_grad = False # spl
            # p.requires_grad = True
        self.view_room_classifier = nn.Sequential(
            nn.Linear(label_length * 512, hidden_size),
            nn.Dropout(dropout_ratio),
            nn.Linear(hidden_size, 31),
            nn.Softmax(dim=1)
        )
        for p in self.view_room_classifier.parameters():
            ...
            # p.requires_grad = True
        self.sm = nn.Softmax(dim=1)
        self.object_attention_layer = WhSoftDotAttention(512, 512)
        self.object_refine_layer = WhSoftDotAttention(512, 512)
        self.room_attention_layer = WhSoftDotAttention(hidden_size, 512)
        ...
        self.object_gcn = GCN_pre_define(512, process_num=num_gcn, max_degree=max_degree, short_cut=short_cut,
                                         load_adj_weight=load_object_relation_weight)
        # num_params = sum(p.numel() for p in self.object_gcn.parameters())
        # print(num_params)
        # exit()
    ...
    def find_nearby(self, text_object_query):
        ...
        attn_object = find_object * values[:, :, None].expand(-1, -1, 512)
    ...
    def _build_current_input(self, u_t_prev, all_u_t, visual_context,
                             h_0, ctx, object_label_set=None, view_label_set=None, action_features=None):
        ...
        # yyy model
        # common_sense part
        object_label_features = self.object_gcn(object_label_set, action_features)
        object_features, _ = self.object_attention_layer(text_refined_object, object_label_features)
        encoded_room_feature = self.encode_label(view_label_set, action_features)
    ...
    def _forward_train(self, u_t_prev, all_u_t, visual_context, input_history, h_0, ctx,
                       ctx_mask, object_label_set=None, view_label_set=None, action_features=None):
        # import ipdb;ipdb.set_trace()
        _input_curent, ctx_pos, text_atten, \
        room_features, g_v, attn_vision, text_alpha, \
        alpha_vision, text_room_class, view_room_loss, alpha_atten_obj, alpha_atten_room, object_index = self._build_current_input(
            u_t_prev, all_u_t,
            visual_context, h_0, ctx,
            object_label_set, view_label_set, action_features)
    ...
    def _forward_search(self, u_t_prev, all_u_t, visual_context, input_history, history_length, h_0, ctx,
                        ctx_mask=None, object_label_set=None, view_label_set=None, gt_labels=None, action_features=None):
