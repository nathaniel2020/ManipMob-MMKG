...
def _build_action_embedding(adj_loc_list, features):
    ...
    feature_embedding = np.zeros((len(adj_loc_list), feature_dim), np.float32)
    for a, adj_dict in enumerate(adj_loc_list):
        ...
        feature_embedding[a, :feature_dim] = features[adj_dict['absViewIndex']]
    ...
    h, w = feature_embedding.shape
    feature_embedding = feature_embedding.mean(axis=0)
    feature_embedding = feature_embedding.reshape(1, w)
...
class R2RBatch():
    ...
    def observe(self, world_states, beamed=False, include_teacher=True, instr_id=None):
        ...
        for i, (state, adj_loc_list) in enumerate(zip(world_states, adj_loc_lists)):
            ...
            action_embedding, feature_embedding = _build_action_embedding(adj_loc_list, feature[0])
            ob = {
                ...
                'action_feature': [feature_embedding]
            }
