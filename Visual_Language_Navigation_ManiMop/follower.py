...
class Seq2SeqAgent(BaseAgent):
    ...
    def _feature_variables_global(self, obs, beamed=False):
        """ Extract precomputed features into variable. """
        feature_lists = [list(zip(*[ob['global_feature'] for ob in (flatten(obs) if beamed else obs)]))]
        assert len(feature_lists) == len(self.env.image_features_list)
        batched = []
        for featurizer, feature_list in zip(self.env.image_features_list, feature_lists):
            batched.append(featurizer.batch_features(feature_list))
    ...
    def _feature_variables_action(self, obs, beamed=False):
        """ Extract precomputed features into variable. """
        feature_lists = list(zip(*[ob['action_feature'] for ob in (flatten(obs) if beamed else obs)]))
        assert len(feature_lists) == len(self.env.image_features_list)
        batched = []
        for featurizer, feature_list in zip(self.env.image_features_list, feature_lists):
            batched.append(featurizer.batch_features(feature_list))
...
class TransformerAgent(Seq2SeqAgent):
    ...
    def _rollout_with_loss(self):
        ...
        for t in range(self.episode_len + 1):
            ...
            f_t_list_global = self._feature_variables_global(obs)
            # f_t_list_action = self._feature_variables_action(obs)
            all_u_t, is_valid, _ = self._action_variable(obs)
