...
from KB.MMKG import GCN_pre_define


class SingleClassifier(LightningModule):

    def __init__(self, cfg, train_ds, val_ds, EMB_SIZE=512, num_gcn=3, max_degree=10, short_cut=False):
        self.optimizer = None
        super().__init__()
        ...
        self.object_gcn = GCN_pre_define(EMB_SIZE, process_num=num_gcn, max_degree=max_degree, short_cut=short_cut)
    ...
    def configure_optimizers(self):
        ...
        if self.cfg['train']['optim'] == 'adam':
            ...
            self.optimizer = torch.optim.AdamW(params_to_optimize, lr=self.cfg['train']['lr'],
                                               weight_decay=self.cfg['train']['weight_decay'])
    ...
    def forward(self, batch):
        (img1_n_feats, img2_n_feats), lang_feats, ans, (key1, key2), annotation, is_visual, annotations = batch

        object_label_set = []
        for annotation in annotations:
            object_label_set.append(self.object_gcn.kg._maybe_register_entity(annotation))
        object_label_set = torch.tensor(object_label_set, dtype=torch.long).to(img1_n_feats.device)
        object_label_features = self.object_gcn(object_label_set)
        object_label_features = object_label_features.squeeze(1)
        ...
        # gcn encoding
        object_label_features = self.gcn_fc(object_label_features)
        ...
        if self.cfg['train']['normalize_feats']:
            ...
            object_label_features = object_label_features / object_label_features.norm(dim=-1, keepdim=True)

        # img1 prob
        img1_enc = self.img_fc(img1_feats)

        img1_prob = self.cls_fc(torch.cat([img1_enc, lang_enc, object_label_features], dim=-1))

        # img2 prob
        img2_enc = self.img_fc(img2_feats)
        img2_prob = self.cls_fc(torch.cat([img2_enc, lang_enc, object_label_features], dim=-1))
